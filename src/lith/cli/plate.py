...
Pure CLI mode (default) prints the rendered prompt and exits. ``--press``
emits a press envelope for a Hermes session or operator; pair it with
...
def main() -> int:
    ...
    parser.add_argument(
        "--press", action="store_true", help="Emit press envelope instead of just printing"
    )
    parser.add_argument(
        "--emit-json", action="store_true", help="With --press, emit machine-readable JSON"
    )
    ...
    # A derived path is a stem: only lith-print sees the bytes that name it.
    print(f"[output]      {out}" + ("" if args.out else ".<jpg|png|webp>"))
    print("Next: pass --press to emit the envelope for image_generate.")
    return 0
